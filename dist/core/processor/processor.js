...
Always prioritizePROCESS over OUTCOME formulations for tasks.`;
        let intent;
        try {
            intent = await this.llmService.processStructured(input, ProcessedIntentSchema, systemPrompt);
        }
        catch (error) {
    ...
    async executeAction(originalInput, intent) {
        ...
        if (action.type === 'create_task') {
            ...
            let subfolder = 'ideas';
            if (intent.classification === 'reflection')
                subfolder = 'reflections';
            if (intent.classification === 'learning')
                subfolder = 'learnings';
    ...
    async appendToDailyLog(input, intent, source) {
