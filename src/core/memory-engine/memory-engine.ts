...
function calcRelevance(days: number, decayRate: number, floor: number): number {
    const r = Math.exp(-decayRate * days);
    return Math.max(floor, Number(r.toFixed(3)));
}

function calcTier(days: number, tiers: MemoryConfig['tiers'], currentTier?: string): string {
    if (currentTier === 'core') return 'core';
    if (days <= tiers.active) return 'active';
    if (days <= tiers.warm) return 'warm';
    if (days <= tiers.cold) return 'cold';
    return 'archive';
}

async function readConfigFile(dirPath: string): Promise<MemoryConfig> {
...
export class MemoryEngine {
    ...
    async getConfig(dirPath: string = this.vaultPath): Promise<MemoryConfig> {
    ...
    async decay(targetDir?: string, dryRun: boolean = false) {
        const dir = targetDir ? path.resolve(targetDir) : this.vaultPath;
        const config = await this.getConfig(dir);
        const cards = await this.findCards(dir, config.skip_patterns);
